...
async function fetchImagesForAds(supabase: SupabaseClient, adIds: string[]): Promise<Map<string, { path: string; sort_order: number }[]>> {
  ...
  try {
    ...
    if (error) {
      ...
      console.error('[SUPABASE ERROR][adsRepo.fetchImagesForAds]', serializeSupabaseError(error));
...
export const adsRepoSupabase: AdsRepository = {
  async list(filters) {
    ...
    if (error) {
      // eslint-disable-next-line no-console
      console.error('[SUPABASE ERROR][adsRepo.list]', serializeSupabaseError(error));
  ...
  async getMostViewed(options) {
    ...
    if (error) {
      // eslint-disable-next-line no-console
      console.error('[SUPABASE ERROR][adsRepo.getMostViewed]', serializeSupabaseError(error));
...
export async function insertAdImages(
  ...
  if (error) {
    // eslint-disable-next-line no-console
    console.error('[SUPABASE ERROR][adsRepo.insertAdImages]', serializeSupabaseError(error));
    throw error;
  }
}
