...
export function Categories() {
  ...
  useEffect(() => {
    setPageTitleAndDescription(
      'Categorias – CeluPublic',
      'Veja as categorias de produtos e serviços disponíveis no CeluPublic.'
    );
  }, []);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <p className="text-muted-foreground">Escolha uma categoria para ver anúncios.</p>
      {withCount.length === 0 ? (
        <Card className="mt-8 border-dashed">
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            <Grid3X3 className="h-12 w-12 text-muted-foreground" />
            <p className="mt-4 text-muted-foreground">Nenhuma categoria cadastrada.</p>
          </CardContent>
        </Card>
      ) : (
