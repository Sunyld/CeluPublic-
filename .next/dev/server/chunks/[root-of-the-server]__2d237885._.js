...
"[project]/Reactjs/Celu/project-bolt-sb1-r7qsskrc/funcional/project/app/api/admin/solicitacoes/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    try {
        ...
        const { data: requests, error: reqError } = await admin.from('seller_requests').select('*').order('created_at', {
            ascending: false
        });
        if (reqError) {
        ...
        const { data: profiles, error: profError } = await admin.from('profiles').select('id, full_name, email, whatsapp').in('id', userIds);
        if (profError) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__2d237885._.js.map